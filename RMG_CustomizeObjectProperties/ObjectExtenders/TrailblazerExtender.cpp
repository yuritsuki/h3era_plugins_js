...
TrailblazerExtender::TrailblazerExtender()
    : ObjectExtender(globalPatcher->CreateInstance("EraPlugin.TrailblazerExtender.daemon_n"))
{
    objectType = eHotaObject::ACTIVE;
    objectSubtype = eHotaObjectActiveType::TRAILBLAZER;
    CreatePatches();
}
...
{
    if (auto trailblazer = GetFromMapItem(mapItem))
    {
...
H3MapItemTrailblazer * TrailblazerExtender::GetFromMapItem(H3MapItem *mapItem) const noexcept
{
