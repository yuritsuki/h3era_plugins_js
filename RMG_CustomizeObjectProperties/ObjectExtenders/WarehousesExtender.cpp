...
namespace warehouses
{
    ...
    {
        if (GetSetupFromMapItem(mapItem))
        {
    ...
    {
        if (auto setup = reinterpret_cast<H3MapItemWarehouse*>(GetSetupFromMapItem(mapItem)))
        {
            *setup = {};
            const UINT resType = mapItem->objectSubtype;
            setup->resourceType = resType;
            if (resType == h3::limits::RESOURCES && !DwordAt(0x27F99AC))
            {
                setup->resourceType = eResource::GOLD;
            }
                                          ...
                                          const BOOL isHuman) const noexcept
    {
        ...
        if (auto setup = reinterpret_cast<H3MapItemWarehouse*>(GetSetupFromMapItem(mapItem)))
        {
            ...
            const INT16 resType = setup->resourceType;
            const INT16 resNum = setup->resourceAmount ? resType == eResource::GOLD ? setup->resourceAmount * 500
                                                                                       : setup->resourceAmount
                                                          : -1;
                                                ...
                                                const H3Position pos) const noexcept
    {
        ...
        if (auto setup = reinterpret_cast<H3MapItemWarehouse*>(GetSetupFromMapItem(mapItem)))
        {
            ...
            if (!isVistedByPlayer || setup->resourceAmount)
            {

                UINT resType = setup->resourceType;
                if (resType >= h3::limits::RESOURCES)
                {
                    resType = eResource::GOLD;
                }
                const INT16 resNum = resType == eResource::GOLD ? setup->resourceAmount * 500 : setup->resourceAmount;
    ...
    // Без подтипа
    void* WarehousesExtender::GetSetupFromMapItem(
        H3MapItem* mapItem) const noexcept
    {
        if (!mapItem ||
            mapItem->objectType != WAREHOUSE_OBJECT_TYPE)
        {
            return nullptr;
        }
    ...
    int WarehousesExtender::AI_OnScouting_Value() const noexcept
    {
        return 10;
    }
}
